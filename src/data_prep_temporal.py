...
if __name__ == "__main__":
    ...
    clean_path    = "data/processed/cycle_level_ready_for_ml_temporal.csv"
    features_path = "data/processed/cycle_level_features_temporal.csv"
    ...
    try:
        ...
        # แปลง Date จาก Buddhist Era → Christian Era
        # เก็บไว้ใน CSV เพื่อใช้ temporal split ใน notebook
        if 'Date' in df.columns:
            df['Date'] = pd.to_datetime(df['Date'], errors='coerce')
            df['Date'] = df['Date'].apply(
                lambda x: x.replace(year=x.year - 543) if pd.notna(x) else x
            )
            print(f"\nDate range (CE): {df['Date'].min()} to {df['Date'].max()}")
            print(f"Missing Date: {df['Date'].isna().sum()}")

        # save clean version (ยังคง Date ไว้)
        os.makedirs(os.path.dirname(clean_path), exist_ok=True)
        df.to_csv(clean_path, index=False)
        print(f"\n✅ Clean data บันทึกที่: {clean_path}")

        # ต่อด้วย feature engineering
        print("\n--- Feature Engineering ---")
        run_feature_engineering(clean_path, features_path)

        print(f"\n✅ เสร็จสมบูรณ์!")
        print("หมายเหตุ: Date column ยังคงอยู่ใน CSV เพื่อใช้ temporal split")
        print("จะถูก drop ใน notebook Block 2 หลังจาก split แล้วเท่านั้น")
